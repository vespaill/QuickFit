...
const exercisesList = (req, res) => {
  Exercise_model.find()
    ...
    .exec((err, exercise) => {
      if (!exercise) {
        return res.status(404).json('No exercises found');
      } else if (err) {
...
const exercisesCreate = (req, res) => {
  ...
  Exercise_model
    ...
    .create(
      {
        /* <model_name>.create.( { <data_to_save> }, <callback> ) */
        name: req.body.name,
...
const exerciseReadOne = (req, res) => {
  Exercise_model
    /* With your route being defined with an URL of '/exercise/:exerciseid' you
       can access the exerciseid parameter from inside the controller like this:
       req.params.exerciseid */

    /* Get an exerciseid from the URL parameters, and pass it to findById() in
       order to tell the Exercise_model what the query will be. */
    .findById(req.params.exerciseid)

    /* Actually execute the query.
       Define callback to accept possible params. */
    .exec((err, exercise) => {
      ...
      /* If Mongoose doesn't error, continue as before, and send an exercise
         object inside an HTTP status response of 200 (which means a successful
         GET or PUT request). */
      return res.status(200).json(exercise);
    });
};
...
const exerciseUpdateOne = (req, res) => {
  ...
  if (!exerciseid) return res.status(404).send('Undefined exercise ID');

  Exercise_model.findById(exerciseid).exec((err, exercise) => {
    ...
    exercise.save((err, updatedExercise) => {
      /* Send an appropriate response, depending on the outcome of the save
         operation. */
      if (err) res.status(404).send(err.message);
      // Return the updated exercise
      else res.status(200).json(updatedExercise);
    });
  });
};
...
const exerciseDeleteOne = (req, res) => {
  const { exerciseid } = req.params;
  if (exerciseid)
    Exercise_model.findByIdAndRemove(exerciseid).exec((err, exercise) => {
      if (err) return res.status(404).send(err.message);
      else if (!exercise) return res.status(404).send('Exercise not found');
      res.status(200).json({
        message: 'Exercise deleted',
        exercise
      });
    });
  else res.status(404).send('Invalid exercise ID');
};
