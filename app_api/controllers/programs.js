...
const programList = (req, res) => {
  Program_model.find()
    ...
    .exec((err, programs) => {
      if (!programs) return res.status(400).send('No programs found');
...
const programAddExercise = (req, res) => {
  ...
  Program_model.findById(req.body.programId).exec((err, program) => {
    if (!program) {
      ...
      return res.status(400).json('Program not found');
    } else if (err) {
    ...
    Exercise_model.findById(req.body.exerciseId).exec((err, exercise) => {
      ...
      program.save((err, updatedProgram) => {
        if (err) {
          ...
          res.status(200).json(updatedProgram.exercises[index]);
        }
      });
    });
  });
...
const programDeleteExercise = (req, res) => {
  if (req.body['programId'] == undefined || req.body['entryId'] == undefined)
    return res.status(400).send('Bad request');

  Program_model.updateOne(
    { _id: req.body.programId },
    { $pull: { exercises: { _id: req.body.entryId } } },
    (err, program) => {
      if (err) {
        return res.status(400).json(err);
      }
