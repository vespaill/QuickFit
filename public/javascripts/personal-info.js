const calcCalories = (weight, height, age, actLvl) => Math.round((66 + 13.7 * weight + 5 * height - 6.8 * age) * actLvl);
const calcProteins = weight => Math.round(weight * 2.205);
const calcFats = weight => Math.round(weight);
const calcCarbs = (caloricGoal, cal_of_proteins, cal_of_fats) => Math.round((caloricGoal - cal_of_proteins - cal_of_fats) / 4);
const debug = false;
