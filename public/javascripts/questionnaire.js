...
let func = function () {
  ...
  /* ------------------------ display question ------------------------ */
  $('#questions-row').html(
    "<h4 class='col-12 text-center' style='font-size:50px'>" +
      questions[i] +
      '</h4>'
  );
  ...
  for (j = 0; j < i; j++) {
    htmlString +=
      "<div class='col-12'><h3 class='text-center'><span class='badge badge-pill'>";
    ...
    if (i == 3) {
      ...
      $('#options-row button:nth-child(1)').on('click', () => {
        window.location.replace(
          `/saveQnrAnswersToUser?goal=${goal}&freq=${freq}&vol=${vol}`
        );
      });
    }
  });
