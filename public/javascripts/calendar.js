    ...
    template: {
      ...
      weekDayname: function (model) {
        ...
        return (
          ...
          model.dayName +
          '</span>' +
          '<span><button class="btn btn-default btn-xs float-right mt-2 mr-2 btn-add-exercise" type="button" data-day="' +
          model.day +
          '"><i class="calendar-icon ic-plus"></i></button></span>'
        );
      },
      monthDayname: function (model) {
        return (
          model.label.toString() +
          '<span><button class="btn btn-default btn-xs float-right btn-add-exercise" type="button" data-day="' +
          model.day +
          '"><i class="calendar-icon ic-plus"></i></button></span>'
        );
      },
      task: function (schedule) {
        return (
          '<div class="bg-' +
          schedule.raw.exercise.group.toLowerCase() +
          '">' +
          schedule.raw.exercise.name +
          ' x ' +
          schedule.raw.reps +
          '</div>'
        );
...
function onClickExercise(schedule) {
  ...
  // Set modal data
  $('#modalViewExerciseLongTitle').text(
    schedule.schedule.raw.exercise.name +
      ' x ' +
      schedule.schedule.raw.reps +
      ', ' +
      schedule.schedule.raw.weight +
      ' lb'
  );
