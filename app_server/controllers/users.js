...
const saveQnrAnswersToUser = (req, res, next) => {
  ...
  } else res.redirect('/register-form');
};
...
const dashboard = (req, res) => {
  ...
  request(
    requestOptions,
    (err, response, { fitness_goal, workout_frequency, workout_volume }) => {
...
const renderCalendar = (req, res, exercises, program) => {
  ...
  if (!(exercises instanceof Array)) {
    ...
    responseBody = [];
  } else if (!exercises.length) message = 'No exercises found in database';
...
const getUserPersonalInfo = (req, res) => {
  ...
  request(
    requestOptions,
    (err, response, { weight, height, gender, age, activityLevel }) => {
...
const updateUserPersonalInfo = (req, res) => {
  debug(
    'updateUserPersonalInfo(): req.session.user._id =',
      ...
      request(
        ...
        (err, { statusCode }, { name }) => {
          ...
          if (statusCode === 200) {
            ...
            debug('400 received');
            /* If true, redirect to the register form, passing an error flag in
               a query string */
          ...
          } else showErr(req, res, statusCode);
        }
      );
    }
  }
...
const renderUserExercises = (req, res, exerciseArray) => {
  ...
  if (!(exerciseArray instanceof Array)) {
    message = 'API lookup error';
    exerciseArray = [];
  } else if (!exerciseArray.length) message = 'User has no excises';
...
const renderExerciseForm = (req, res) => {
  ...
  res.render('exercise-form', {
    ...
    equipOptions: [
      { abbrv: 'BB', name: 'Barbell' },
      { abbrv: 'DB', name: 'Dumbbell' },
      { abbrv: 'C', name: 'Cable' },
      { abbrv: 'M', name: 'Machine' },
      { abbrv: 'BE', name: 'Body weight<br>& additional equipment' },
      { abbrv: 'BW', name: 'Body weight only' }
    ],
...
const createUserExercise = (req, res) => {
  ...
  if (!postdata.name || !postdata.equip || !postdata.group) {
    if (!postdata.name) res.redirect('/dashboard/exercise-list/add?err=name');
    if (!postdata.equip) res.redirect('/dashboard/exercise-list/add?err=equip');
    if (!postdata.group) res.redirect('/dashboard/exercise-list/add?err=group');
  } else {
    request(requestOptions, (err, { statusCode }, { name }) => {
      /* Check to see whether the status is 400, the body has a name, and that
         name is ValidationError. If true, redirects to the exercise form,
         passing an error flag in a query string */
      if (statusCode === 200)
